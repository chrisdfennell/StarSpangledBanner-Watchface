...
if ($Export) {
    ...
    $outputPath = Join-Path $PSScriptRoot "bin\StarSpangledBanner.iq"
    & $monkeyc -e -f $junglePath -o $outputPath -y $keyPath
} else {
    Write-Host "Building for device: $Device..." -ForegroundColor Cyan
    $outputPath = Join-Path $PSScriptRoot "bin\StarSpangledBanner.prg"
...
if ($Export) {
    Write-Host "Package Succeeded! Output: bin\StarSpangledBanner.iq" -ForegroundColor Green
} else {
    Write-Host "Build Succeeded! Output: bin\StarSpangledBanner.prg" -ForegroundColor Green
}
